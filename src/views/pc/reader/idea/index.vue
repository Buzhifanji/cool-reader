...
watch(() => bookStore.readingBook.chapter, (newValue, oldValue) => {
  if (newValue !== oldValue) {
    const list = notesStore.bookIdeas.filter(value => value.chapter === bookStore.readingBook.chapter);
...
<template>
  <n-list hoverable clickable v-if="notesStore.bookIdeas.length">
    <n-list-item v-for="item in notesStore.bookIdeas" :key="item.id" @click="jumpByChapter(item.chapter)">
      <template #suffix>
